    ...
    int offset)
{
    ...
    if (!new_str || !leftover)
        return ((ft_safe_free((unsigned char **)&leftover), ft_safe_free((unsigned char **)&value), ft_safe_free((unsigned char **)&new_str), ft_safe_free((unsigned char **)&temp_left), NULL));
    (ft_safe_free((unsigned char **)&leftover), ft_safe_free((unsigned char **)&value), ft_safe_free((unsigned char **)&str_token), ft_safe_free((unsigned char **)&temp_left));
    return (new_str);
}
...
char *refine_name_var(char *token_name, char *result)
{
   ...
   // e_printf("\nRESULT = %s$\n", result);
    while (result[i])
    {
        if (result[i] == '"' || result[i] == ' ' || result[i] == '$' || (result[i] >= '\t' && result[i] <= '\r'))
            break;
        i++;
    }
    result[i] = '\0';
